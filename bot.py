...
# Import only necessary API keys from config.py (no TTSOPENAI_API_KEY)
from config import TELEGRAM_BOT_TOKEN, GEMINI_API_KEY, PLAYHT_API_KEY # PLAYHT_API_KEY is retained for Play.ht TTS

import io
import requests # Import requests for API calls (used for Play.ht)
...
def generate_story_with_ai(prompt: str, full_markdown_mode: bool = False) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        if full_markdown_mode:
            ...
            response = model.generate_content(f"Generate a story with a title and body based on the following prompt: {prompt}\n\nTitle:",
                                              generation_config=genai.types.GenerationConfig(
                                                  candidate_count=1,
                                                  max_output_tokens=2000,
                                                  temperature=0.7
...
SUPPORTED_LANGUAGES = {
    ...
    "Assamese": "as", "Kashmiri": "ks", "Konkani": "kok", "Maithili": "mai", "Manipuri": "mni",
    ...
    "Chhattisgarhi": "hne", "Awadhi": "awa", "Marwari": "mwr", "Bundeli": "bns", "Bagheli": "bgp",
    "Kangri": "xnr", "Kumaoni": "kuma", "Garhwali": "gbm", "Himachali": "him", "Dogri (India)": "dgo",
    "Kashmiri (India)": "kas", "Nepali (India)": "nep", "Manipuri (India)": "mni", "Bodo (India)": "brx",
    "Santali (India)": "sat", "Santhali (India)": "sat", "Khasi (India)": "kha", "Mizo (India)": "lus",
    "Garo (India)": "grt", "Kokborok (India)": "kok", "Bhojpuri (India)": "bho", "Haryanvi (India)": "bgc",
    "Rajasthani (India)": "raj", "Magahi (India)": "mag", "Chhattisgarhi (India)": "hne", "Awadhi (India)": "awa",
    "Marwari (India)": "mwr", "Bundeli (India)": "bns", "Bagheli (India)": "bgp", "Kangri (India)": "xnr",
...
# --- Play.ht API Integration (Retained, as per your last provided code) ---
def generate_audio_with_playht(text: str, voice_id: str) -> str | None:
    ...
    headers = {
        ...
        "X-User-Id": "YOUR_USER_ID", # Play.ht often requires a User ID, replace with actual if available
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    help_message = (
        ...
        "I'll ask you for a prompt. Just type your idea, like 'a detective solving a mystery in space' "
        ...
        "I'll give you options like Funny, Serious, or Fantasy. Choose one, then provide your topic! 🎭\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    selected_language = context.user_data.get('selected_language', 'en') # Default to English
...
Respond only with the final story in Markdown. Do not include instructions, tags, or notes.'''
        ...
        if story_markdown:
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await update.message.reply_text("📦 Your content is ready to download:", reply_markup=reply_markup)
        ...
        if age_match:
            try:
                ...
                # This should ideally not happen if re.search finds a valid number, but as a safeguard
                await update.message.reply_text("Invalid age. Please enter a number for the age (e.g., 30, 65): 🎂")
                context.user_data['state'] = 'waiting_for_tts_age' # Stay in this state until valid age is provided
        else:
            await update.message.reply_text("Invalid age. Please enter a number for the age (e.g., 30, 65): 🎂")
            context.user_data['state'] = 'waiting_for_tts_age' # Stay in this state until valid age is provided
        return # Exit early as this path handles its own response and state reset
        ...
        PLAYHT_VOICES = {
            'male': {
                'young': "s3://voice-cloning-zero-shot/d9ff7a5d-9241-4a36-a57c-3b1077c3866e/original/manifest.json", # Example Play.ht voice ID
                'adult': "s3://voice-cloning-zero-shot/8d42422c-122c-422f-806f-222222222222/original/manifest.json", # Example Play.ht voice ID
                'senior': "s3://voice-cloning-zero-shot/e0f2f2f2-f2f2-f2f2-f2f2-f2f2f2f2f2f2/original/manifest.json" # Example Play.ht voice ID
            },
            'female': {
                'young': "s3://voice-cloning-zero-shot/a1b2c3d4-e5f6-7890-1234-567890abcdef/original/manifest.json", # Example Play.ht voice ID
                'adult': "s3://voice-cloning-zero-shot/b2c3d4e5-f6a7-8901-2345-67890abcdef0/original/manifest.json", # Example Play.ht voice ID
        ...
        if gemini_response_body:
            ...
            if audio_file_path:
                ...
                await update.message.reply_text("Sorry, I couldn't generate the audio from Gemini's response. Please try again. (Play.ht issue)")
        else:
...
Respond only with the game/challenge content in Markdown. Do not include instructions, tags, or notes.'''
    ...
    # This block handles all content generation (story, ebook, writer) that results in text output
    if title and body:
        context.user_data['last_content'] = {
            'title': title,
            'body': body,
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data.startswith('lang_'):
        ...
        if previous_state == 'waiting_for_story_language_selection':
            keyboard = [
                [InlineKeyboardButton("Short (approx. 250 words)", callback_data='length_short')],
                [InlineKeyboardButton("Medium (approx. 500 words)", callback_data='length_medium')],
                [InlineKeyboardButton("Long (approx. 1000 words)", callback_data='length_long')],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(f"Language set to {lang_code.upper()}. Now, choose your desired story length: 📏", reply_markup=reply_markup)
            context.user_data['state'] = 'waiting_for_story_length_selection'
        elif previous_state == 'waiting_for_ebook_language_selection':
            await query.edit_message_text(f"Language set to {lang_code.upper()}. What topic should your educational ebook be about? For example: 'Quantum Physics' or 'The History of Ancient Rome'. 💡")
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(f"Language set to {lang_code.upper()}. Now, choose a tone for your story:", reply_markup=reply_markup)
            context.user_data['state'] = 'waiting_for_writer_tone'
        else:
            await query.edit_message_text("Language selected, but an unexpected state occurred. Please try again with a command like /story.")
            context.user_data['state'] = None
