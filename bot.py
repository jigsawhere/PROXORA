...
  reply_markup = InlineKeyboardMarkup(keyboard)  welcome_message = (
...
"Kokborok": "kok", "Bhojpuri": "bho", "Haryanvi": "bgc", "Rajasthani": "raj","Magahi": "mag",
...
row.append(InlineKeyboardButton(lang,
callback_data=f'lang_{SUPPORTED_LANGUAGES[lang]}'))
keyboard.append(row)
  reply_markup = InlineKeyboardMarkup(keyboard)
await update.message.reply_text("Choose a language for your content: 🌐",
reply_markup=reply_markup)
...
-> Noneawait send_language_selection(update, context,
...
reply_markup = InlineKeyboardMarkup(keyboard)
await update.message.reply_text("Choose a voice for Text-to-Speech: 🗣️",
reply_markup=reply_markup)
...
\n\n{bonus_status}")
 async def watchad_command(update: Update, context:
...
in space' ""and I'll craft a unique tale for you! ✨\n\n"
...
> None:
await update.message.reply_text("🎮 Welcome to the Gamified Creative Mode!
What kind of game or interactive story would you like to play? (e.g., 'a mystery
...
await update.message.reply_text(f"✅ Data received from WebApp: `{data}`",
parse_mode='Markdown')
else:
...
English
  if state == 'waiting_for_detailed_story_prompt':story_length = context.user_data.get('story_length', 'short') # Default to short
word_count = 250
if story_length == 'medium':
word_count = 500
elif story_length == 'long':
word_count = 1000
  await update.message.reply_text(f"Generating your {story_length} story in
...
if len(story_markdown) > 2000: # Telegram message limit is 4096, but 2000 issafer for readability + buttons
display_text = story_markdown[:1900] + "\n\n...[Story continues in
download]..."
await update.message.reply_text(display_text, parse_mode='Markdown')
else:
await update.message.reply_text(story_markdown,
parse_mode='Markdown')
...
reply_markup = InlineKeyboardMarkup(keyboard)
await update.message.reply_text("📦 Your story is ready to download:",
reply_markup=reply_markup)
  else:
await update.message.reply_text("Sorry, I couldn't generate your detailed
story. Please try a different prompt.")
...
await update.message.reply_text(f"Generating a story in
{selected_language.upper()} about: {user_text}... Please wait! ⏳")
title, body = retry_story_generation(f"Generate a story in {selected_language}
about: {user_text}", 1)
content_type = "story"
  elif state == 'waiting_for_ebook_topic':
await update.message.reply_text(f"Generating an ebook in
{selected_language.upper()} about: {user_text}... Please wait! ⏳")
...
reply_markup = InlineKeyboardMarkup(keyboard)
await update.message.reply_text("📦 Your ebook is ready to download:",
reply_markup=reply_markup)
else:
await update.message.reply_text("Sorry, I couldn't generate your ebook.
Please try a different topic.")
  context.user_data['state'] = None
if 'selected_language' in context.user_data:
del context.user_data['selected_language']
return # Exit early as this path handles its own response and state reset
  elif state == 'waiting_for_writer_topic':tone = context.user_data.get('writer_tone')
selected_language = context.user_data.get('selected_language', 'en')
await update.message.reply_text(f"Generating a {tone} story in
{selected_language.upper()} about: {user_text}... Please wait! ⏳")
title, body = retry_story_generation(f"Generate a story in {selected_language}
...
await update.message.reply_text(f"You entered age {age}. Now, please send
me the text you want to convert to speech. 🎤")
except ValueError:
# This should ideally not happen if re.search finds a valid number, but as a
safeguard
await update.message.reply_text("Invalid age. Please enter a number for the
age (e.g., 30, 65): 🎂")
context.user_data['state'] = 'waiting_for_tts_age' # Stay in this state until
valid age is provided
else:
await update.message.reply_text("Invalid age. Please enter a number for the
age (e.g., 30, 65): 🎂")
...
documentation)
PLAYHT_VOICES = {
'male': {'young': "s3://voice-cloning-zero-shot/d9ff7a5d-9241-4a36
...
voice_id = PLAYHT_VOICES[tts_voice]['adult'] # Default to adult if no age
provided
  if not voice_id:
await update.message.reply_text("Sorry, an invalid voice or age combination
was selected. Please try again with /speak.")
context.user_data['state'] = None
return
  await update.message.reply_text(f"Thinking and generating audio with
{tts_voice} voice (age {tts_age if tts_age else 'default'})... Please wait! 🎤")
...
following text, subtly or directly incorporating the concept of 'Slow Toxin' andensuring the tone is appropriate for a {tts_voice} voice of approximately {tts_age if
...
caption=f"Here's Gemini's response in {tts_voice} voice!")
os.remove(audio_file_path)
else:
await update.message.reply_text("Sorry, I couldn't generate the audio from
Gemini's response. Please try again.")
else:
await update.message.reply_text("Sorry, Gemini couldn't generate a response
for that. Please try again with a different text.")
  context.user_data['state'] = None
if 'tts_voice' in context.user_data:
del context.user_data['tts_voice']
if 'tts_age' in context.user_data:
del context.user_data['tts_age']
return # Exit early as this path handles its own response and state reset
  elif state == 'waiting_for_play_prompt':
selected_language = context.user_data.get('selected_language', 'en')
await update.message.reply_text(f"Generating a gamified creative experience in
{selected_language.upper()} based on: {user_text}... Please wait! 🎮")
...
- Keep the response concise, encouraging further interaction.- Format in Markdown.
...
reply_markup = InlineKeyboardMarkup(keyboard)
await update.message.reply_text("What's your next move? Or try another
command!", reply_markup=reply_markup)
else:
await update.message.reply_text("Sorry, I couldn't generate a gamified
experience for that. Please try a different prompt.")
  context.user_data['state'] = None
if 'selected_language' in context.user_data:
del context.user_data['selected_language']
return # Exit early as this path handles its own response and state reset
...
reply_markup = InlineKeyboardMarkup(keyboard)  await update.message.reply_text(response_message,
reply_markup=reply_markup)
else:
await update.message.reply_text("Sorry, I couldn't generate content for that.
...
context.user_data['state'] = 'waiting_for_ebook_topic'elif previous_state == 'waiting_for_writer_language_selection':
keyboard = [
...
await query.edit_message_text(f"Your content has been sent as a
...
application.add_handler(CommandHandler("story", story_command))application.add_handler(CommandHandler("ebook", ebook_command))
